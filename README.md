...
Реализована полноценная поддержка импорта/экспорта данных в трех форматах: **JSON, YAML и CSV**.

### Поддерживаемые форматы:

| Формат | Класс | Библиотека | Расширения |
|--------|-------|------------|------------|
| **JSON** | `JsonDataHandler` | Jackson 2.x | .json |
| **YAML** | `YamlDataHandler` | Jackson-dataformat-yaml | .yaml, .yml |
| **CSV** | `CsvDataHandler` | OpenCSV | .csv |

- Фабрика для выбора нужного обработчика
- Автоматическое определение формата по расширению файла
- Поддержка .json, .yaml, .yml, .csv
